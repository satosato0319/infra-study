...
# nginx
## 詰まった点
※ 検証のため、nginx 関連ファイルを削除し、2回目の構築を実施した。
...
```
## 確認したこと
### 1つ目： 2回目構築時、nginx 起動失敗

- 以下を実行したが設定ファイルが存在しなかった
```bash
sudo nginx -t
```
- また、`/etc/nginx/nginx.conf` の存在を確認したが、ファイルが存在しなかった
```bash
ls /etc/nginx/nginx.conf
```
- nginxは設定ファイルを読み込んで起動するため、設定ファイル不在が起動失敗の要因と判断した
...
## 対応
### 1つ目： nginx設定ファイル不在への対応

- 再インストールしたが設定ファイルが復元されなかった
- そのため、 フォルダとその配下にファイルを作成し、最低限の設定を記述した
...
```
### 2つ目： 権限エラーへの対応
- sudo をつけるの忘れていたため、再度 sudo をつけて実行したら、確認できた。
...
```
- 上記ログから、公開鍵がサーバーに置くことができなかったことがわかった。そのため、まず`sshd_config` を確認した結果、パスワード認証が無効化されていた
```bash
...
ls ~/.ssh
```
- パスワード認証が無効になっており、`ssh-copy-id` が利用できなかった為、手動で公開鍵を `authorized_keys` に追加する必要があると判断
- 
## 対応
...
```
- `.ssh` ディレクトリを自分のみアクセス可能、 `aurhorized_keys` ファイルは自分のみ読み書きができるように権限を設定
```bash
...
# api
## 詰まった点
### 1つ目： `python3 -m venv .venv` を実行しようとしたが、`ensurepip is not available` とエラーメッセージが表示され、仮想環境作成に失敗した。
### 2つ目： `render_template("index.html")` 実行時に `index.html` が見つからず、画面表示に失敗した。
...
### 2つ目： index.html 不在
- Flask は `render_template("index.html")` で指定したファイル名を探すため、ファイル名のスペルミスに注意する。
